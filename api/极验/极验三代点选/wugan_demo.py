...
class Jiyan_v3_wugan():
    ...
    def run(self):
        ...
        verify_info = self.verify_web()
        self.verify_demo(verify_info)
        # print(self.get_click_task())
        # print(self.verify_click())

    def verify_demo(self,verify_info):
        verify_info = verify_info['data']
        data = {
            'geetest_challenge': self.challenge,
            'geetest_validate': verify_info.get("validate"),
            'geetest_seccode': f'{verify_info.get("validate")}|jordan'
        }

        response = requests.post('https://www.geetest.com/demo/gt/validate-fullpage',data=data)
        print(response.text)


if __name__ == '__main__':
    v3 = Jiyan_v3_wugan("2ba7ec69d4faab852e993d7f1f1b59f3", "ce33de396f8d04030f6eca8fbd225070")
    # v3.chushihua()
    v3.run()
